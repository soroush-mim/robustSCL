...
class PGDConsMultiWithOrg(PGD):
    ...
    def forward(self, images, labels, loss):
      ...
      for step in range(self.steps):
          adv_images.requires_grad = True
          outputs = self.model(adv_images)
          f1, f2, f3 = torch.split(outputs, [bsz, bsz, bsz], dim=0) #f1 and f2 -> torch.Size([bsz, 128]
          outputs = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1), f3.unsqueeze(1)], dim=1) 
