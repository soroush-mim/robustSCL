...
def train(train_loader, model, criterion, optimizer, epoch, opt, atk):
    """one epoch training"""
    model.train()
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        if ADV_TRAINING:
                loss = trades_loss(model=model,
                       ...
                       epsilon=opt.epsilon,
                       perturb_steps=opt.num_steps_train,
                       beta=opt.beta,
                       trainmode=opt.trainmode,
                       fixbn=opt.fixbn,
                       fixmode=opt.fixmode)
        else:
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # train for one epoch
        time1 = time.time()
        loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, opt, atk)
        time2 = time.time()
